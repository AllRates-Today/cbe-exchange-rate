# Central Bank of Egypt Exchange Rate API — cbe-exchange-rate

[![npm version](https://img.shields.io/npm/v/cbe-exchange-rate.svg)](https://www.npmjs.com/package/cbe-exchange-rate)
...
- 🏛️ **Official published rates** — Central Bank of Egypt's own table, with the publisher's own `rate_date` on every response
...
```bash
yarn add cbe-exchange-rate
```

```bash
pnpm add cbe-exchange-rate
```

Also published under the org scope as [`@allratestoday/cbe-exchange-rate`](https://www.npmjs.com/package/@allratestoday/cbe-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Central Bank of Egypt rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'cbe-exchange-rate';
...
```js
import { getRatesForDate } from 'cbe-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'EGP' });
```
...
If Central Bank of Egypt does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via EGP from two published rates |
...
| `404` | Pair or date range not covered by Central Bank of Egypt |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'cbe-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('cbe-exchange-rate');

getRate('USD', 'EGP', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Central Bank of Egypt rates page](https://allratestoday.com/central-bank-rates-api/cbe/) — live table, publication cadence, FAQ
...
- [GitHub](https://github.com/AllRates-Today/cbe-exchange-rate)
