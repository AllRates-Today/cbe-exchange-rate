# Central Bank of Egypt Exchange Rate API client

Official **Central Bank of Egypt** (Egypt) daily exchange rates in Node.js / TypeScript — 18 currencies against the EGP, with history back to 2026. Zero dependencies, works in Node 18+, Bun, Deno, and edge runtimes (uses global `fetch`).
...
```js
import { getRate, getLatestRates } from 'cbe-exchange-rate';

// One pair at the official Central Bank of Egypt rate
const pair = await getRate('USD', 'EGP', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // e.g. USD -> EGP on the bank's own date
...
```js
import { getRatesForDate, getHistory } from 'cbe-exchange-rate';
...
const series = await getHistory(
  { source: 'USD', target: 'EGP', from: '2026-01-01' },
  { apiKey: 'art_live_...' }
);
```
...
If Central Bank of Egypt does not print a pair directly, the API resolves it from the bank's table (inverse, or a cross rate via EGP) and flags it `derived: true` with the `method` — so official and computed values are never confused.
